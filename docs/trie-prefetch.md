...
A *build-time* node relayout was also explored separately and rejected (it cost
~14x build time for ≤1.10x lookup); this runtime approach is the answer that
pays off without touching the format or the build.
...
SUDACHI_BENCH_DICT=/tmp/full.dic SUDACHI_BENCH_TRIALS=31 \
  cargo run -p sudachi --release --example tokenize_pipeline_bench
```

`tokenize_pipeline_bench` runs the scalar and pipelined paths interleaved and
reports min/median/CV, so the speedup is measured against the same build.
